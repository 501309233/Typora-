 ...
 * excel导出潜在客户
 */
public Workbook export(String fileName,List<ProspectiveModel>prospectiveModelList) throws IOException {
    //excel格式是.xlsx
    ...
    listTitle.add("来源");
    listTitle.add("状态");
    for (int i =0; i<prospectiveModelList.size();i++){
        //存放excel内容数据
        ArrayList<String>listBody = new ArrayList<String>();
        //存放数据顺序和存放标题顺序对应
        listBody.add(prospectiveModelList.get(i).getCode());
        listBody.add(prospectiveModelList.get(i).getName());
        listBody.add(prospectiveModelList.get(i).getMobile());
        listBody.add(prospectiveModelList.get(i).getResource().getName());
        listBody.add(prospectiveModelList.get(i).getStatus().getName());
        list.add(listBody);
    }
    //调用公共类的导出方法
